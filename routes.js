...
// Express 4 doesn't automatically catch async errors.
// Wrap async handlers so DB errors don't crash the process (Railway would show 502).
...
function slNowColombo() {
  ...
  const fmt = new Intl.DateTimeFormat('en-CA', {
    ...
    minute: '2-digit',
    hour12: false,
    // Prevent some runtimes returning "24" for midnight
    hourCycle: 'h23'
  }).formatToParts(now);
...
router.get('/security/pending', authMiddleware, requireRole('SECURITY'), asyncHandler(async (req,res)=>{
  const { rows } = await query(
    `SELECT l.*, u.emp_no, u.full_name
     FROM leaves l
     LEFT JOIN users u ON u.id=l.employee_id
...
router.get('/security/code/:code', authMiddleware, requireRole('SECURITY'), asyncHandler(async (req,res)=>{
...
router.post('/security/code/:code/out', authMiddleware, requireRole('SECURITY'), asyncHandler(async (req,res)=>{
  const code = String(req.params.code || '').trim();
  if(!/^[0-9]{6}$/.test(code)) return res.status(400).json({ message:'Invalid code (6 digits)' });
...
router.post('/security/code/:code/in', authMiddleware, requireRole('SECURITY'), asyncHandler(async (req,res)=>{
  const code = String(req.params.code || '').trim();
  if(!/^[0-9]{6}$/.test(code)) return res.status(400).json({ message:'Invalid code (6 digits)' });
  const now = slNowColombo();
